// This header file contains the explanation and purpose of the functions inside the ML_Strategy class
...
// This macro is used to export the ML_Strategy class to the dynamic link library (DLL) that is loaded by the trading engine.
#ifdef _WIN32
...
/**
 * Below are header files that are used by the ML_Strategy class. We just tell the compiler to look for these files.
...
// Class declaration
class ML_Strategy : public Strategy {
public:
    // Constructor & Destructor functions for this class
    ML_Strategy(StrategyID strategyID,
...
public:
    ...
    // Below are event handling functions that the 'ML_Strategy' class overrides to receive trading events from the trading engine.

    /**
    * OnBar is a member function of the ML_Strategy class that gets called by the backtesting framework in Strategy Studio whenever a bar event occurs for an instrument that the strategy is subscribed to.
    ...
    **/
    ...
    /**
    * The OnTrade function is a member function of the ML_Strategy class and is called by the backtesting framework in Strategy Studio whenever a trade event occurs for an instrument that the strategy is subscribed to.
    ...
    */
    ...
    /**
     * The OnOrderUpdate function is a member function of the ML_Strategy class in the trading strategy code. It is triggered whenever a completed order is detected and computes the quantityHeld.
    ...
    */
    ...
    /**
     * The OnParamChanged function is a member function of the ML_Strategy class in the Strategy Studio.
...
/**
 * The ML_Strategy class uses the private access modifier multiple times to declare different types of member functions and member variables.
 * The first private section declares two private helper functions specific to the ML_Strategy class, named AdjustPortfolio() and SendOrder().
 * The second private section declares two virtual functions that are being overridden by the ML_Strategy class to register for strategy events and define the strategy parameters.
 * The third private section declares several private member variables used to store the state and data of the strategy.
 * The reason for using private access modifiers is to encapsulate the internal workings of the ML_Strategy class and prevent other classes or functions from accessing or modifying its private members directly.
...
private: 
     ...
     * It is a virtual function in the base class that is overridden by the ML_Strategy class to register for the specific events that it needs.
     ...
     * Inside the function, the ML_Strategy class can register for any events that it needs using the provided eventRegister object.
    */
    virtual void RegisterForStrategyEvents(StrategyEventRegister* eventRegister, DateType currDate); 

    /**
     * DefineStrategyParams() is a virtual function in the ML_Strategy class.
...
private:
    // private member variables of the ML_Strategy class.
...
extern "C" {
    ...
    **/
     ...
     * The function is defined as part of the ML_Strategy class, which is a derived class of the Strategy base class
     * The function returns a const char pointer, which points to the string "ML_Strategy".
    ...
    /**
     * Creates an instance of the ML_Strategy class
     * Checks whether the provided strategyType matches the type of the ML_Strategy class using the GetType() function
     * Returns a pointer to the new ML_Strategy object if there is a match, or a null pointer if there is no match
     * This function is necessary for the Strategy Studio to create and manage instances of the ML_Strategy class during backtesting and simulation.
    */
    _STRATEGY_EXPORTS IStrategy* CreateStrategy(const char* strategyType,
        ...
        if (strcmp(strategyType, GetType()) == 0) {
            return *(new ML_Strategy(strategyID, strategyName, groupName));
        } else {
            return NULL;
        }
    }
