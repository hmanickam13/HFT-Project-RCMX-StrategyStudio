...
class HelloWorldClearStrategy : public Strategy {
public:

    HelloWorldClearStrategy(StrategyID strategyID,
        const std::string& strategyName,
        const std::string& groupName);
    ~HelloWorldClearStrategy();


public: /* from IEventCallback */
    /**
    * OnBar is a member function of the Test_Strategy class that gets called by the backtesting framework in Strategy Studio whenever a bar event occurs for an instrument that the strategy is subscribed to.
    ...
    **/
    ...
    /**
    * The OnTrade function is a member function of the Test_Strategy class and is called by the backtesting framework in Strategy Studio whenever a trade event occurs for an instrument that the strategy is subscribed to.
    ...
    */
    ...
    /**
     * The OnOrderUpdate function is a member function of the Test_Strategy class in the trading strategy code. It is triggered whenever a completed order is detected and computes the quantityHeld.
    ...
    */
    ...
    /**
     * The OnParamChanged function is a member function of the Test_Strategy class in the Strategy Studio.
...
/**
 * The Test_Strategy class uses the private access modifier multiple times to declare different types of member functions and member variables.
 * The first private section declares two private helper functions specific to the Test_Strategy class, named AdjustPortfolio() and SendOrder().
 * The second private section declares two virtual functions that are being overridden by the Test_Strategy class to register for strategy events and define the strategy parameters.
 * The third private section declares several private member variables used to store the state and data of the strategy.
 * The reason for using private access modifiers is to encapsulate the internal workings of the Test_Strategy class and prevent other classes or functions from accessing or modifying its private members directly.
...
private:  // Helper functions specific to this strategy
     ...
     * The functionis called by the trading logic to make adjustments to the portfolio holdings and can be customized by the user to implement their own portfolio management strategies.
    **/
...
private: /* from Strategy */
     ...
     * It is a virtual function in the base class that is overridden by the Test_Strategy class to register for the specific events that it needs.
     ...
     * Inside the function, the Test_Strategy class can register for any events that it needs using the provided eventRegister object.
    */
    virtual void RegisterForStrategyEvents(StrategyEventRegister* eventRegister, DateType currDate); 
    
    /**
     * DefineStrategyParams() is a virtual function in the Test_Strategy class.
...
extern "C" {
    ...
    **/
     ...
     * The function is defined as part of the Test_Strategy class, which is a derived class of the Strategy base class
     * The function returns a const char pointer, which points to the string "Test_Strategy".
    ...
    _STRATEGY_EXPORTS const char* GetType() {
        return "HelloWorldClearStrategy";
    }

    /**
     * Creates an instance of the Test_Strategy class
     * Checks whether the provided strategyType matches the type of the Test_Strategy class using the GetType() function
     * Returns a pointer to the new Test_Strategy object if there is a match, or a null pointer if there is no match
     * This function is necessary for the Strategy Studio to create and manage instances of the Test_Strategy class during backtesting and simulation.
    */
    _STRATEGY_EXPORTS IStrategy* CreateStrategy(const char* strategyType,
        ...
        if (strcmp(strategyType, GetType()) == 0) {
            return *(new HelloWorldClearStrategy(strategyID, strategyName, groupName));
        } else {
            return NULL;
        }
    }
