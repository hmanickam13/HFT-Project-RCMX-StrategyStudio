// This header file contains the explanation and purpose of the functions inside the Helloworld_Strategy class
...
// This macro is used to export the Helloworld_Strategy class to the dynamic link library (DLL) that is loaded by the trading engine.
#ifdef _WIN32
...
/**
 * Below are header files that are used by the Helloworld_Strategy class. We just tell the compiler to look for these files.
...
// Class declaration
class Helloworld_Strategy : public Strategy {
public:
    // Constructor & Destructor functions for this class
    Helloworld_Strategy(StrategyID strategyID,
...
public:
    ...
    // Below are event handling functions that the 'Helloworld_Strategy' class overrides to receive trading events from the trading engine.

    /**
    * OnBar is a member function of the Helloworld_Strategy class that gets called by the backtesting framework in Strategy Studio whenever a bar event occurs for an instrument that the strategy is subscribed to.
    ...
    **/
    ...
    /**
    * The OnTrade function is a member function of the Helloworld_Strategy class and is called by the backtesting framework in Strategy Studio whenever a trade event occurs for an instrument that the strategy is subscribed to.
    * A trade event refers to a specific occurrence related to a trade of a financial instrument, such as a stock or a commodity.
    * It could include events like the execution of a buy or sell order, the update of trade details, or any other event that involves the trading of an instrument.
    ...
    * If a buy or sell order is executed, the function updates the stock size, total_revenue, and trade times variables accordingly.
    * The function also calculates the capital of the strategy at each trade event by adding the value of the stock holdings and total_revenue.
    ...
    */
    ...
    /**
     * OnScheduledEvent is a virtual member function of a strategy class that is called when a scheduled event occurs during the backtesting process.
     * It receives a parameter msg of type ScheduledEventMsg, which contains information about the scheduled event, including its ID, type, scheduled time, and additional data.
     * The purpose of the function is to handle the scheduled event and perform specific actions or logic associated with that event.
     * By overriding this function, the user can customize the behavior of the strategy when a scheduled event occurs.
     * Inside the function, the strategy can perform various operations based on the event type and associated data.
     * The function allows the strategy to incorporate time-based events into its decision-making process and adapt its behavior accordingly.
     * Examples of actions that can be performed in OnScheduledEvent include making trading decisions, adjusting parameters or indicators, updating strategy state, or triggering specific actions at predefined intervals.
     * The function provides flexibility and customization options to execute time-dependent trading strategies, manage periodic updates or calculations, or trigger specific actions at scheduled intervals.
     * The strategy can utilize the information from the ScheduledEventMsg to determine the appropriate response to the event.
     * The OnScheduledEvent function is one of the callback functions in the strategy class that allows the strategy to interact with the backtesting framework and respond to specific events.
    */
   virtual void OnScheduledEvent(const ScheduledEventMsg& msg);

    /**
     * The OnOrderUpdate function is a member function of the Helloworld_Strategy class in the trading strategy code. It is triggered whenever a completed order is detected and computes the quantityHeld.
    ...
    */
    ...
    /**
     * The OnParamChanged function is a member function of the Helloworld_Strategy class in the Strategy Studio.
...
/**
 * The Helloworld_Strategy class uses the private access modifier multiple times to declare different types of member functions and member variables.
 * The first private section declares two private helper functions specific to the Helloworld_Strategy class, named AdjustPortfolio() and SendOrder().
 * The second private section declares two virtual functions that are being overridden by the Helloworld_Strategy class to register for strategy events and define the strategy parameters.
 * The third private section declares several private member variables used to store the state and data of the strategy.
 * The reason for using private access modifiers is to encapsulate the internal workings of the Helloworld_Strategy class and prevent other classes or functions from accessing or modifying its private members directly.
...
private:  
     ...
     * It updates the stock size and total_revenue variables of the strategy based on the trades executed and the current market conditions.
    ...
    **/
     ...
     * The function executes the trade by sending the order to the market and updates the stock size and total_revenue variables of the strategy based on the executed trade.
...
private: 
     ...
     * It is a virtual function in the base class that is overridden by the Helloworld_Strategy class to register for the specific events that it needs.
     ...
     * Inside the function, the Helloworld_Strategy class can register for any events that it needs using the provided eventRegister object.
    */
    virtual void RegisterForStrategyEvents(StrategyEventRegister* eventRegister, DateType currDate); 

    /**
     * DefineStrategyParams() is a virtual function in the Helloworld_Strategy class.
...
private:
    // private member variables of the Helloworld_Strategy class.
    ...
    int total_stock_position; // the number of shares of the instrument currently held in the strategy's portfolio.
...
extern "C" {
    ...
    **/
     ...
     * The function is defined as part of the Helloworld_Strategy class, which is a derived class of the Strategy base class
     * The function returns a const char pointer, which points to the string "Helloworld_Strategy".
    ...
    _STRATEGY_EXPORTS const char* GetType() {
        return "Helloworld_Strategy";
    }

    /**
     * Creates an instance of the Helloworld_Strategy class
     * Checks whether the provided strategyType matches the type of the Helloworld_Strategy class using the GetType() function
     * Returns a pointer to the new Helloworld_Strategy object if there is a match, or a null pointer if there is no match
     * This function is necessary for the Strategy Studio to create and manage instances of the Helloworld_Strategy class during backtesting and simulation.
    */
    _STRATEGY_EXPORTS IStrategy* CreateStrategy(const char* strategyType,
        ...
        if (strcmp(strategyType, GetType()) == 0) {
            return *(new Helloworld_Strategy(strategyID, strategyName, groupName));
        } else {
            return NULL;
        }
    }
